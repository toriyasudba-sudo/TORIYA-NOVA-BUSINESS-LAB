...
async function validateInitData(initData, env) {
  if (!env.BOT_TOKEN) return { error: 'bot_token_missing' };
  if (!initData) return { error: 'init_data_missing' };

  const original = new URLSearchParams(initData);
  const receivedHash = original.get('hash');
  if (!receivedHash) return { error: 'hash_missing' };

  const buildCheck = (excludeSignature = false) => {
    const params = new URLSearchParams(initData);
    params.delete('hash');
    if (excludeSignature) params.delete('signature');
  ...
  let valid = false;
  for (const excludeSignature of [false, true]) {
    const expected = hex(await hmacSha256(new Uint8Array(secret), buildCheck(excludeSignature)));
    if (safeEqual(receivedHash.toLowerCase(), expected.toLowerCase())) {
      valid = true;
      break;
    }
  }
  if (!valid) return { error: 'hash_mismatch' };

  const maxAge = Number(env.INIT_DATA_MAX_AGE_SEC || 86400);
  const authDate = Number(original.get('auth_date') || 0);
  if (!authDate) return { error: 'auth_date_missing' };
  if (Math.abs(Date.now() / 1000 - authDate) > maxAge) return { error: 'init_data_expired' };

  try {
    const user = JSON.parse(original.get('user') || 'null');
    if (!user?.id) return { error: 'user_missing' };
    return { user };
  } catch {
    return { error: 'user_parse_failed' };
  }
}
...
async function getMembership(userId, env) {
  if (!env.BOT_TOKEN) return { allowed: false, error: 'bot_token_missing' };
  if (!env.CHANNEL_ID) return { allowed: false, error: 'channel_id_missing' };
  try {
    ...
    return { allowed, status: member.status || 'unknown' };
  } catch (e) {
    const msg = String(e?.message || 'telegram_api_error');
    console.error('getChatMember', msg);
    let error = 'telegram_api_error';
    if (/unauthorized/i.test(msg)) error = 'bot_token_invalid';
    else if (/chat not found/i.test(msg)) error = 'channel_not_found';
    else if (/member list is inaccessible|not enough rights|administrator/i.test(msg)) error = 'bot_not_admin';
    else if (/user not found/i.test(msg)) error = 'telegram_user_not_found';
    return { allowed: false, error, detail: msg.slice(0, 180) };
  }
}
...
async function verifiedUser(body, env, requireMembership = true) {
  const check = await validateInitData(body.initData || '', env);
  if (check.error) return { error: check.error };
  const user = check.user;
  if (requireMembership) {
    const m = await getMembership(user.id, env);
    if (!m.allowed) return { error: m.error || 'not_member', user, membership: m };
  }
  return { user };
}
...
async function apiAccess(request, env) {
  try {
    ...
    const membership = await getMembership(v.user.id, env);
    return json(membership.error ? 502 : 200, {
      allowed: membership.allowed,
      error: membership.error || (membership.allowed ? null : 'not_member'),
      detail: membership.detail || null,
