...
async function apiLead(req,res){
  try{
    ...
    const d=body.diagnostic||{};const overall=Math.max(0,Math.min(100,Number(d.overall)||0));const weak=String(d.weak||'').slice(0,40);const weakScore=Math.max(0,Math.min(100,Number(d.weakScore)||0));
    const names={context:'Контекст бизнеса',client:'Клиент и JTBD',competition:'Конкурентное поле',journey:'Путь клиента',architecture:'Архитектура и MVP',ai:'Готовность к AI'};
    ...
    const text=`🔥 <b>Заявка в практическую группу</b>\n\n👤 <a href="${userLink}">${esc(fullName)}</a> · ${esc(username)}\n🆔 <code>${v.user.id}</code>\n\n📐 Общая ясность: <b>${overall}%</b>\n⚠️ Слабое звено: <b>${esc(names[weak]||weak||'не определено')} — ${weakScore}%</b>\n\nСтарт: 5 сентября · 10 человек · 7 000 ₽`;
...
const server=http.createServer(async(req,res)=>{
  if(req.method==='POST'&&req.url.startsWith('/api/access'))return apiAccess(req,res);
  if(req.method==='POST'&&req.url.startsWith('/api/event'))return apiEvent(req,res);
  if(req.method==='POST'&&req.url.startsWith('/api/lead'))return apiLead(req,res);
  if(req.method==='GET'||req.method==='HEAD')return staticFile(req,res);
  res.writeHead(405);res.end('Method not allowed');
});
server.listen(PORT,()=>console.log(`TORIYA NOVA Business Lab: http://localhost:${PORT}`));
